      ...
      <div className="max-w-4xl mx-auto">
              ...
              <div className="space-y-6">
                {messages.map((msg, index) => (
                  <div key={index} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-xs lg:max-w-md transform transition-all duration-300 hover:scale-105 ${msg.role === "user" ? "order-2" : "order-1"}`}>
                      <div className={`flex items-center mb-2 ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-medium ${msg.role === "user" ? "bg-gradient-to-r from-blue-500 to-blue-600" : "bg-gradient-to-r from-orange-500 to-red-500"}`}>
                          {msg.role === "user" ? "U" : "AI"}
                        </div>
                      ...
                      <div className={`px-4 py-3 rounded-2xl shadow-lg border ${msg.role === "user" ? "bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-br-md border-blue-200" : "bg-white text-gray-800 rounded-bl-md border-gray-200"}`}>
        ...
        <div className="bg-white bg-opacity-90 rounded-2xl shadow-xl border border-white border-opacity-30 p-6 backdrop-blur-sm">
          <div className="flex flex-col lg:flex-row items-center justify-center gap-6">
            ...
            <div className="flex flex-col items-center">
              <button
                ...
                className={`relative group flex items-center justify-center w-20 h-20 rounded-full font-medium text-white shadow-2xl transform transition-all duration-300 hover:scale-110 active:scale-95 focus:outline-none focus:ring-4 focus:ring-opacity-50 ${isRecording ? "bg-gradient-to-r from-red-500 to-red-600 cursor-not-allowed focus:ring-red-300 animate-pulse" : "bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 focus:ring-orange-300"}`}
              >
                {isRecording ? (
                  <div className="flex flex-col items-center">
          ...
          <div className="mt-6 flex items-center justify-center">
            <div className={`inline-flex items-center space-x-3 px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 ${isRecording ? "bg-red-100 text-red-800 border-2 border-red-200" : isLoading ? "bg-orange-100 text-orange-800 border-2 border-orange-200" : "bg-green-100 text-green-800 border-2 border-green-200"}`}>
              <div className={`w-3 h-3 rounded-full ${isRecording ? "bg-red-500 animate-pulse" : isLoading ? "bg-orange-500 animate-spin border-2 border-orange-300 border-t-transparent" : "bg-green-500"}`}></div>
              <span>
